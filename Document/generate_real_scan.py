...
async def main():
    ...
    server_params = StdioServerParameters(
        command="docker",
        args=["run", "-i", "--rm", "document-mcp-server"],
        env=None
    )
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # Prepare placeholders with real scan data
            placeholders = {
                "{nomor}": "002",
                "{bulan}": "11",
                ...
                "{scope}": "Penilaian dilakukan pada aplikasi web testphp.vulnweb.com yang terletak di alamat IP 44.228.249.3. Scope mencakup pemindaian port, deteksi kerentanan web, dan pengujian SQL injection pada parameter yang ditemukan.",
            ...
            # Call the document_generate_word tool
            result = await session.call_tool(
                "document_generate_word",
                content="",
